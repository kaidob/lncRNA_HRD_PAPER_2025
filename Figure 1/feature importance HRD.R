...
perm_test <- vip::vi_permute(
  ...
  target = "HRD_Score",  # Specify the target column
  metric = "rmse", 
  nsim = 100, 
...
test_predictions <- predict(rf_fit, test_data) %>% bind_cols(test_data)
metrics <- yardstick::metrics(test_predictions, truth = HRD_Score, estimate = .pred)
print(metrics)
