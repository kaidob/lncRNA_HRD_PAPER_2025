...
# Filter breast cancer samples and select relevant columns, now focusing on HRD_Score
hrd_data <- DR %>% 
  filter(type == "OV") %>% 
  select( HRD_Score,  # Change target to HRD_Score
          ENSG00000255651.2,
          ENSG00000234264.1,
          ENSG00000272172.1,
          ENSG00000260369.2,
...
dim(hrd_data)
head(hrd_data)
summary(hrd_data$HRD_Score)  # Check HRD_Score summary

# Implement a three-way split: train (60%), validation (20%), test (20%)
set.seed(123)
# Initial split to get the test set
initial_split_obj <- initial_split(hrd_data, prop = 0.8, strata = HRD_Score)  # Use HRD_Score for strata
training_validation <- training(initial_split_obj)
testing <- testing(initial_split_obj)

# Split the training data into training and validation
set.seed(234)
train_val_split <- initial_split(training_validation, prop = 0.75, strata = HRD_Score)  # Use HRD_Score for strata
...
# Create a proper preprocessing recipe for HRD_Score
hrd_recipe <- recipe(HRD_Score ~ ., data = training) %>%  # Change target to HRD_Score
...
set.seed(345)
cv_folds <- vfold_cv(training, v = 10, strata = HRD_Score)  # Use HRD_Score for stratification
...
# Define tuning grids (same as before)
...
# Set control object for the tuning grid
...
evaluate_model <- function(model_fit, new_data, model_name) {
  predictions <- predict(model_fit, new_data) %>%
    bind_cols(new_data %>% select(HRD_Score))
  
  metrics <- metric_set(rmse, rsq, mae)
  results <- metrics(predictions, truth = HRD_Score, estimate = .pred)
  
  # Create visualization
  p <- ggplot(predictions, aes(x = HRD_Score, y = .pred)) +
...
lm_pearson <- cor(lm_val$predictions$HRD_Score, lm_val$predictions$.pred)
rf_pearson <- cor(rf_val$predictions$HRD_Score, rf_val$predictions$.pred)
svm_pearson <- cor(svm_val$predictions$HRD_Score, svm_val$predictions$.pred)
...
ggplot(all_predictions, aes(x = HRD_Score, y = .pred, color = model)) +
...
# Define the target variable (HRD_Score in this case)
target_variable <- "HRD_Score"

# Extract the fitted Random Forest model from the workflow
rf_model <- extract_fit_parsnip(rf_fit)

# Preprocess the training data using the finalized recipe from the workflow
preprocessed_training <- hrd_recipe %>%
  prep() %>%
  bake(new_data = training)  # Apply preprocessing steps

# Split into predictors (X) and target (y)
train_x <- preprocessed_training %>% select(-HRD_Score)
train_y <- preprocessed_training$HRD_Score

# Create the prediction wrapper for the extracted ranger model.
# Note: We use `data` instead of `newdata`, and extract predictions from the "predictions" field.
pred_wrapper <- function(object, newdata) {
  pred <- predict(object, data = newdata)
  return(as.numeric(pred$predictions))
}

# Run feature importance calculation using vip with the permutation method.
# Here, we pass the extracted model (rf_model) instead of the workflow.
importance_permutations <- vip(
  rf_model,           # Use the extracted ranger model
  train = train_x,    # Preprocessed predictors only
  target = train_y,   # Target vector (numeric)
  method = "permute", # Permutation method
  metric = "rmse",    # Evaluation metric
  pred_wrapper = pred_wrapper,
  num_features = 29  # Show top 20 features
)
